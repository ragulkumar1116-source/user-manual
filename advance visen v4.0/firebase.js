...
export function addCustomer(data) {
  ...
  return push(custRef, data);
}

export function updateCustomer(key, data) {
  const custRef = ref(db, `customers/${key}`);
  return update(custRef, data);
}

export function deleteCustomer(key) {
  const custRef = ref(db, `customers/${key}`);
  return remove(custRef);
}
...
export function listenToCustomers(callback) {
  ...
  onValue(custRef, (snapshot) => {
    ...
    console.error("Firebase customer fetch error:", error);
  });
}

export { db, ref, onValue, set, get, remove, update, push };